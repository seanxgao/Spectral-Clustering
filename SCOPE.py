...
def best_cut_finder(adj, gpu: bool, sparse: bool) -> int:
    ...
    def _best_cut_sparse_csp(M):
        # ensure GPU CSR
        if _HAS_CUPY and isinstance(M, csp.csr_matrix):
            csr = M
        elif sp.issparse(M):
            cpu = M.tocsr()
            csr = csp.csr_matrix((cp.asarray(cpu.data, dtype=cp.float32),
                                 ...
                                 shape=cpu.shape)
        elif _HAS_CUPY and isinstance(M, cp.ndarray):
...
def treebuilder(L, thre = None, indices=None, parallel = True, manager = None):
    ...
    # === Recursive construction (choose parallel/serial based on parallel parameter) ===
    ...
    # Top-level entry: open thread pool for parallel; run directly for serial
    if parallel:
...
def matrixtype(L, sparse: bool, gpu: bool):
    """
    Convert L to one of the following four types (determined only by sparse / gpu):
    ...
    Convention: sp, cp, csp have been successfully imported:
      import scipy.sparse as sp
      import cupy as cp
      import cupyx.scipy.sparse as csp
    Also assumes numpy as np has been imported
    """
    # ==== Target: GPU + SPARSE -> cupyx.scipy.sparse.csr_matrix ====
    if gpu and sparse:
        if _HAS_CUPY and csp.isspmatrix(L):
            return L.tocsr()                         # Already GPU sparse -> CSR
        if sp.issparse(L):
            return csp.csr_matrix(L)                 # CPU sparse -> copy to GPU
        if _HAS_CUPY and isinstance(L, cp.ndarray):
            return csp.csr_matrix(L)                 # GPU dense -> GPU CSR
        # Others treated as CPU dense / array-like
        return csp.csr_matrix(cp.asarray(L))         # CPU dense -> GPU CSR

    # ==== Target: GPU + DENSE -> cupy.ndarray ====
    if gpu and not sparse:
        if _HAS_CUPY and isinstance(L, cp.ndarray):
            return L                                  # Already GPU dense
        if _HAS_CUPY and csp.isspmatrix(L):
            return L.toarray()                        # GPU sparse -> GPU dense (cupy)
        if sp.issparse(L):
            return cp.asarray(L.toarray())            # CPU sparse -> CPU dense -> GPU dense
        # Others treated as CPU dense / array-like
        return cp.asarray(L)                          # CPU dense -> GPU dense

    # ==== Target: CPU + SPARSE -> scipy.sparse.csr_matrix ====
    if (not gpu) and sparse:
        if sp.issparse(L):
            return sp.csr_matrix(L)                   # Already CPU sparse -> CSR
        if _HAS_CUPY and csp.isspmatrix(L):
            return L.get().tocsr()                    # GPU sparse -> CPU sparse
        if _HAS_CUPY and isinstance(L, cp.ndarray):
            return sp.csr_matrix(cp.asnumpy(L))       # GPU dense -> CPU sparse
        # Others treated as CPU dense / array-like
        return sp.csr_matrix(L)                       # CPU dense -> CPU sparse

    # ==== Target: CPU + DENSE -> numpy.ndarray ====
    # gpu=False and sparse=False
    if sp.issparse(L):
        return L.toarray()                            # CPU sparse -> CPU dense (numpy)
    if _HAS_CUPY and csp.isspmatrix(L):
        return L.get().toarray()                      # GPU sparse -> CPU dense (numpy)
    if _HAS_CUPY and isinstance(L, cp.ndarray):
        return cp.asnumpy(L)                          # GPU dense -> CPU dense
    # Others treated as CPU dense / array-like
    return np.asarray(L)                               # Ensure it's numpy.ndarray
...
def sparse_score(L):
    ...
    elif _HAS_CUPY and csp.isspmatrix(L):  # GPU sparse
        s = float(cp.asnumpy(L.diagonal().sum(dtype=cp.float64)))
    elif _HAS_CUPY and isinstance(L, cp.ndarray):  # GPU dense
        s = float(cp.asnumpy(L.diagonal().sum(dtype=cp.float64)))
